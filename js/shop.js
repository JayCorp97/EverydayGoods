...
(function () {
  ...
  /* --- Static catalog fallback ---------------------------------------
     When there is no Node backend (e.g. deployed as static files), the
     /api/* routes 404. For read-only catalog data we fall back to
     data-static.json so the pages still render. Cart, login and orders
     genuinely need the server and will stay empty. */
  let staticCatalog = null;
  async function loadStatic() {
    if (!staticCatalog) {
      staticCatalog = await fetch("data-static.json").then((r) => r.json());
    }
    return staticCatalog;
  }

  async function catalogFallback(url) {
    const cat = await loadStatic();
    const path = url.split("?")[0];
    const qs = new URLSearchParams(url.split("?")[1] || "");

    if (path === "/api/categories") return cat.categories;

    if (path === "/api/products") {
      const category = qs.get("category");
      const needle = (qs.get("q") || "").toLowerCase().trim();
    ...
    const single = path.match(/^\/api\/products\/(\d+)$/);
    if (single) {
      const p = cat.products.find((x) => x.id === Number(single[1]));
      if (p) return p;
    }

    return null; // not a catalog route — let the original error stand
  }
  ...
  async function api(method, url, body) {
    ...
    try {
      ...
      const fb = method === "GET" ? await catalogFallback(url) : null;
      if (fb) return fb;
      throw networkError;
    }

    if (!res.ok && method === "GET") {
      const fb = await catalogFallback(url);
      if (fb) return fb;
    }
